...
processNSCs <- function (rawData, cvLimitSample = 0.25, cvLimitTube = 0.05, forceIntercept = FALSE) {
  ...
  #--------------------------------------------------------------------------------------
  rawData [['MassSugarM']] <- (rawData [['MeanAbsorbance490']] - rawData [['InterceptSugar']]) /
                               rawData [['SlopeSugar']]
  rawData [['MassSugarB']] <- (rawData [['Absorbance490_Blank']] - rawData [['InterceptSugar']]) /
                               rawData [['SlopeSugar']]


  # Correct sugar amount [mg] for background signal using no phenol
  #--------------------------------------------------------------------------------------
  rawData [['MassSugar']] <- (rawData [['MassSugarM']] - rawData [['MassSugarB']]) *
                              rawData [['DilutionFactorSugar']] * rawData [['VolumeSugar']]

  # Determine amount of strach [mg] from absorbance
  #--------------------------------------------------------------------------------------
  rawData [['MassStarch']] <- (rawData [['MeanAbsorbance525']] - rawData [['InterceptStarch']]) /
                               rawData [['SlopeStarch']]
  rawData [['MassStarch']] <- rawData [['MassStarch']] * rawData [['DilutionFactorStarch']] *
                              rawData [['VolumeStarch']]

  # Convert mass to concentrations [mg g-1]
  #--------------------------------------------------------------------------------------
  rawData [['ConcentrationSugarMgG']]  <- rawData [['MassSugar']]  / rawData [['MassSample']]
  rawData [['ConcentrationStarchMgG']] <- rawData [['MassStarch']] / rawData [['MassSample']]
